...
def outgoing_transactions(request):
    ...
    if request.method == 'POST':
        ...
        return HttpResponse("todo ha ido bien, solo falta que implementes las nominas")  # QUITAR CUANDO TODO FUNCIONE
    else:
...
def incoming_transactions(request):
    ...
    try:
        ...
        try:
            ...
            return HttpResponseBadRequest("The account you tried to send money to does not exist")
        
        taxed_amount = calc_commission(amount, 'INCOMING')
        total_amount = amount - taxed_amount
        account.balance += total_amount
        account.save()
        new_transaction = Transaction.objects.create(agent=sender, account=account, concept=concept, amount=amount)
